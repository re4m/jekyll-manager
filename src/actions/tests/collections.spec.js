...
describe('Actions::Collections', () => {
  ...
  it('creates the post with autogenerated filename from current date', () => {
    ...
    return store.dispatch(actions.putDocument('create', new_post_without_date.collection, ''))
      .then(() => {
        expect(store.getActions()).toEqual(expectedActions);
      });
  });
  ...
  it('creates the post from a draft', () => {
    ...
    return store.dispatch(actions.putDocument('publish', new_post_with_date.collection, ''))
      .then(() => {
        expect(store.getActions()).toEqual(expectedActions);
      });
  });
  ...
  it('creates the post in a subdirectory from a draft', () => {
    nock(API)
      .put(`/collections/${new_post_with_date.collection}/test/${new_post_with_date.path}`)
      .reply(200, post);
    ...
    return store.dispatch(actions.putDocument('publish', new_post_with_date.collection, 'test', ''))
      .then(() => {
        expect(store.getActions()).toEqual(expectedActions);
      });
  });
